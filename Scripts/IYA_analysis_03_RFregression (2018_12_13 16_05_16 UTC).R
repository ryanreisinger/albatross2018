...
setwd("D:/PEI_Toppredators/IYA/Data_Original")
list.files()
...
# dat5 <- read.dbf("D:/PEI_Toppredators/IYA/Data_Newi_2/89057.dbf", as.is = T)
# dat5 <- dat5[ , c("PTT", "DATE", "TIME", "LAT1", "LONG1", "LC")]
# names(dat5) <- c("id", "date", "time", "lat", "lon", "lc")
# dat5$date <- paste(dat5$date, dat5$time, sep = " ")
# dat5$time <- NULL
# dat5$date <- as.POSIXct(dat5$date, format = "%Y-%m-%d %H:%M:%S", "GMT")
# dat5$date <- format(dat5$date, "%Y/%m/%d %H:%M:%S")

dat <- rbind.data.frame(dat1, dat2, dat3, dat4, dat5)
dat <- dat[!duplicated(dat), ]#remove duplicate lines
write.csv(dat, "D:/PEI_Toppredators/IYA/Working/dataAllCombined_02.csv", row.names = F)
rm(list = ls())
...
library(PBSmapping)
library(devtools)
install_github("ianjonsen/bsam", force = TRUE)
library(bsam)
...
for(i in 1:length(ids2)){
  ...
  slat <- rep(-46.637866, nrow(d)) #distance from Prince Edward rather than first loc
  slon <- rep( 37.946053, nrow(d)) #distance from Prince Edward rather than first loc
  s <- as.matrix(cbind(slon, slat))
  e <- as.matrix(cbind(d$lon, d$lat))
  d$dis <- distGeo(s, e)/1000
  d$dmax <- max(d$dis)
  
  d$dif <- difftime(d$date, d.start, units = "days")
  d$dif <- as.numeric(d$dif)
  
  d$bearing <- c(NA, bearing(p1 = cbind(d$lon[1:length(d$lon)-1], d$lat[1:length(d$lat)-1]),
                       p2 = cbind(d$lon[2:length(d$lon)], d$lat[2:length(d$lat)])))
  
  plot(y = d$dis, x = d$dif, main = d$id[1])
  abline(h = 5, col = "red")
  abline(h = 10, col = "red")
  abline(h = 20, col = "red")
  abline(h = 50, col = "red")
...
#----------------------------------
#summary table
birdsForTable <- c("89052", "89058", "89061", "89062", "89065", "89066", "89067", "89068",
                   "105290", "105291", "105292", "105293", "105294", "105295", "105296", "105297")

fits.sum <- fits.up[!duplicated(fits.up$id), c("year", "id", "start", "end", "dur", "dmax")]
fits.sum <- fits.sum[fits.sum$id %in% birdsForTable, ]
write.csv(fits.sum, file = "summaryUpdated.csv", row.names = F)

#-----------------------------------
#Define trips and make an additional summary table
birdsForTable <- c("89052", "89058", "89061", "89062", "89065", "89066", "89067", "89068",
  "105290", "105291", "105292", "105293", "105294", "105295", "105296", "105297")
 
tr <- fits.up[fits.up$id %in% birdsForTable, ]
...
for (j in 1:length(birdsForTable)){
  print(birdsForTable[j])
  test <- tr[tr$id == birdsForTable[j], ]
  test$d <- test$dif
  plot(test$d, test$dis, type = "l") +
  points(test$d, test$dis, pch = 19)
  abline(h=10)
  t <- locator(7)$x
  #t <- t[!duplicated(t)]
...
tr2$trip2 <- paste0(tr2$id, "_", tr2$trip)
saveRDS(tr2, "trips.RDS")
tr2 <- readRDS("trips.RDS")

trippers <- unique(tr2$trip2)

dt <- list()
for(i in 1:length(trippers)){
  d <- tr2[tr2$trip2 == trippers[i], ]
  d.start <- min(d$date)
  d.end <- max(d$date)
  dur <- d.end - d.start
  dur <- as.character(dur)
...
trips.frame <- do.call(rbind, dt)

trips.sum <- trips.frame[!duplicated(trips.frame$trip2), c("year", "id", "trip", "start", "end", "dur", "dmax")]
trips.sum <- trips.sum[!is.na(trips.sum$trip), ]
#trips.sum <- trips.sum[!(trips.sum$dur < 2 | trips.sum$dmax < 20), ]

write.csv(trips.sum, file = "trips_summary.csv", row.names = F)

#------------------------------

# Trip-based latitutde plot
trips.frame <- trips.frame[!is.na(trips.frame$trip), ]
trips.frame$triptype <- NA

for (i in 1:nrow(trips.frame)) {
  trip <- trips.frame$trip2[i]
  howfar <- max(trips.frame[trips.frame$trip2 == trip, "dmax"])
  if (howfar > 1200) {
    trips.frame$triptype[i] <- "far"
  } else {
    trips.frame$triptype[i] <- "near"
  }
}

ggplot(trips.frame, aes(x = lat)) +
  geom_histogram(binwidth = 1, fill="gray20", colour="gray20", alpha = 0.5, position = "identity", center = 42.5) +
  theme_bw() +
  ...
  labs(y = "Number of location estimates", x = "Latitude") +
  facet_wrap(year~triptype, ncol = 1, scales = "free_y") +
  scale_x_continuous(breaks = seq(-50, -33, 2)) #+
expand_limits(x=c(-50, -33)) 
...
# Get EEZ
dir <- "D:/PEI_Toppredators/Datavis&Mapping"
eez = readOGR(dsn=dir, layer="eez")
eez@data$id = rownames(eez@data)
eez.points = fortify(eez, region="id")

dir <- "D:/PEI_Toppredators/Datavis&Mapping"
pei = readOGR(dsn=dir, layer="PEI_EEZ")
pei@data$id = rownames(pei@data)
pei.points = fortify(pei, region="id")

dir <- "D:/PEI_Toppredators/Datavis&Mapping"
mpa = readOGR(dsn=dir, layer="PEI_MPA")
mpa@data$id = rownames(mpa@data)
mpa.points = fortify(mpa, region="id")

#Fronts
library(orsifronts)
myOrsi <- fortify(orsifronts)
myOrsi <- myOrsi[myOrsi$id %in% c("stf", "saf"), ]
...
p2 <- p2 + scale_fill_gradient(low="gray70", high="white", name = "Depth (m)",
                               guide = guide_colorbar(title.position = "top")) #if land is NA
p2 <- p2 + geom_path(data=myOrsi, aes(x=long, y=lat, group = group), linetype = 1, size = 0.3, colour = "gray40")
p2 <- p2 + geom_point(data = fits.copy, aes(x = fits.copy$lon, y = fits.copy$lat, colour = b), size = 0.9) +
  facet_grid(. ~ year) +
  scale_colour_viridis(name = "Behavioural state",
                      guide = guide_colorbar(title.position = "top"))
p2 <- p2 + coord_quickmap(expand = FALSE)
...
p2 <- p2 + geom_path(data = eez.points, aes(long, lat, group = group), color = "black")
p2 <- p2 + geom_path(data = pei.points, aes(long, lat, group = group), color = "black")
#p2 <- p2 + geom_path(data = mpa.points, aes(long, lat, group = group), color = "black")
...
#TIFF
tiff(file = "overmap2.tiff", width = 6.7, height = 4, units = "in", res = 800,
     antialias = "cleartype", compression = "lzw")
p2
dev.off()

##Faceted by individual

#plot only 11 individuals in table
fits.copy.copy <- fits.copy[fits.copy$id == "89052" |
                         fits.copy$id == "89057" |
                         fits.copy$id == "89058" |
                         fits.copy$id == "89061" |
                         fits.copy$id == "89062" |
...
# OR, choose only individuals selected by Newi
fits.copy.copy <- fits.copy[fits.copy$id == "89052" |
                              fits.copy$id == "89061" |
                              fits.copy$id == "89066" |
                              fits.copy$id == "89068" |
...
p3 <- ggplot(data = b4.p, aes(x=lon, y=lat))
p3 <- p3 + geom_tile(aes(fill=val))
p3 <- p3 + scale_fill_gradient(low="gray70", high="white", name = "Depth (m)",
                               guide = guide_colorbar(title.position = "top")) #if land is NA
p3 <- p3 + geom_point(data = fits.copy.copy, aes(x = fits.copy.copy$lon, y = fits.copy.copy$lat, colour = fits.copy.copy$b), size = 0.8) + facet_grid(. ~ year) +
  scale_colour_viridis(name = "Behavioural state",
                      guide = guide_colourbar(title.position = "top"))
p3 <- p3 + facet_wrap(~ id, ncol = 4)
p3 <- p3 + coord_quickmap(expand = FALSE)
p3 <- p3 + theme_bw(base_size = 8) + theme(panel.grid.minor=element_blank(),
                                           ...
                                           strip.text = element_text(colour = "black", size = 8),
                                           axis.title.y = element_blank())
#p3 <- p3 + geom_path(data = eez.points, aes(long, lat, group = group), color = "black")
#p3 <- p3 + geom_path(data = pei.points, aes(long, lat, group = group), color = "black")
#p3 <- p3 + geom_path(data = mpa.points, aes(long, lat, group = group), color = "black")
world <- borders("world", colour="gray20", fill = "gray20", xlim = c(min(fits.copy$lon), max(fits.copy$lon)), ylim = c(min(fits.copy$lat), max(fits.copy$lat)))
p3 <- p3 + world + xlim(minx, maxx) + ylim(miny, maxy) + annotate("point", x = 37.94, y = -46.64, pch = 16, size = 3)
p3

tiff(file = "panelmap.tiff", width = 6.7, height = 5, units = "in", res = 800,
     antialias = "cleartype", compression = "lzw")
p3
dev.off()


#Plot short and long trips in panels
#trips.frame <- trips.frame[trips.frame$lon < 45, ]
...
p2 <- p2 + scale_fill_gradient(low="gray70", high="white", name = "Depth (m)",
                               guide = guide_colorbar(title.position = "top")) #if land is NA
p2 <- p2 + geom_path(data=myOrsi, aes(x=long, y=lat, group = group), linetype = 1, size = 0.3, colour = "gray40")
p2 <- p2 + geom_point(data = trips.frame, aes(x = lon, y = lat, colour = b), size = 0.9, inherit.aes = F) +
  facet_grid(triptype ~ year) +
  scale_colour_viridis(name = "Behavioural state",
                       guide = guide_colorbar(title.position = "top"))
p2 <- p2 + coord_quickmap(expand = FALSE)
p2 <- p2 + theme_bw(base_size = 8) + theme(panel.grid.minor=element_blank(),
...
#TIFF
tiff(file = "overmapSplitTrips.tiff", width = 6.7, height = 7, units = "in", res = 800,
     antialias = "cleartype", compression = "lzw")
p2
dev.off()
...
#Bin map
  ggplot(fits.copy, aes(y = lat, x = dif)) +
    ...
    theme(panel.grid.major.y = element_blank(),
          ...
          strip.background = element_blank(),
          axis.text = element_text(colour = "black")) +
    labs(y = "Latitude", x = "Days since departure") +
    facet_wrap(~year, ncol = 1) +
    scale_x_continuous(breaks = seq(-50, -33, 2)) #+
  expand_limits(x=c(-50, -33)) 

#Displacement map
  ggplot(fits.copy, aes(y = lat, x = dif, group = id)) +
    #geom_smooth(data = fits.copy, aes(y = lat, x = dif), inherit.aes = F) +
    #geom_bin2d(binwidth = 2) +
    geom_path(alpha = 0.5) +
    theme_bw() +
    theme(panel.grid.major.y = element_blank(),
...
#Polar plot for leaving directions
  leave <- fits.copy[fits.copy$dif < 2, ]
  leave <- leave[!is.na(leave$bearing), ]
  leave$bearing[leave$bearing<0]<-360+leave$bearing[leave$bearing<0]
  
  p8 <- ggplot(leave, aes(x = bearing, fill = year))
  p8 <- p8 + geom_histogram(binwidth=10, origin=0)
  p8 <- p8 + coord_polar(start=0) +
    scale_x_continuous(limits=c(0,360), breaks=seq(0, 360, by=45),
                       minor_breaks=seq(0, 360, by=15))
  p8 <- p8 + theme_bw() + theme(panel.border = element_blank(),
                                panel.grid.major = element_line(colour = "black"),
                                panel.grid.minor = element_blank(),
                                axis.text = element_text(colour = "black"))
  p8 <- p8 + labs(x = "Heading (degrees)",
                  y = "Number of location estimates")
  p8 <- p8 + scale_fill_manual(values = c("grey30", "grey60"))
  p8
#------------------------------
## 05. Modelling
...
fits.comp <- fits.env.sub[ , c(21:33)]
fits.comp <- fits.comp[complete.cases(fits.comp), ]
library(corrplot)
fits.cor <- cor(fits.comp, method = "spearman")
corrplot(fits.cor, type = "lower", "number")
rm(fits.comp)
#Drop DIST.200, EKE, PROD
...
rf1 <- randomForest(formula = as.factor(state) ~ DEPTH + SST +
                      SSHA + WINDU + WINDV + CHL +SSTgrad + SSHgrad,
...
#random forest oi continuous b-estimate
library(randomForest)

rf1 <- randomForest(formula = b ~ DEPTH + SST +
                      SSHA + WINDU + WINDV + CHL +SSTgrad + SSHgrad,
                    data = fits.env.sub, na.action = "na.omit",
                    keep.inbag = T, keep.forest = T, ntree = 1000, importance = F)
rf1
...
#model selection
library(AUCRF)
d.rf <- fits.env.sub[ , c("state", "DEPTH", "SST", "SSTgrad", "SSHA", "SSHgrad", "WINDU", "WINDV", "CHL")]
d.rf <- d.rf[complete.cases(d.rf), ]
d.rf$state <- as.factor(d.rf$state)

rf.auc <- AUCRF(formula = state ~ .,
                data = d.rf, pdel = 0, keep.inbag = T, keep.forest = T, n.tree = 1000)


library(forestFloor)
X <- X <- d.rf[ ,-1]
#X <- X[complete.cases(X), ]
ff = forestFloor(rf1, X ,binary_reg = F,calc_np=T)
Col = fcol(ff,cols=1,outlier.lim = 2.5)
#plot_seq argument control variables to plot
plot(ff,col="grey", plot_GOF = T, orderByImportance=TRUE, GOF_args = list(col="black"), plot_seq = c(1, 2, 3, 4))
