...
p2 <- p2 + geom_point(data = fits.copy, aes(x = fits.copy$lon, y = fits.copy$lat, colour = as.factor(state), shape = as.factor(state)), size = 0.9) +
  facet_grid(. ~ year) +
  scale_colour_manual(values = c("#009E73", "#000000", "#616161"), name = "Behavioural state",
                      guide = guide_legend(title.position = "top")) +
  scale_shape_manual(values = c(19, 1, 3), name = "Behavioural state",
                     guide = guide_legend(title.position = "top"))
...
#TIFF
tiff(file = "overmap.tiff", width = 6.7, height = 4, units = "in", res = 800,
...
p3 <- p3 + scale_fill_gradient(low="gray70", high="white", name = "Depth (m)") #if land is NA
p3 <- p3 + geom_point(data = fits.copy.copy, aes(x = fits.copy.copy$lon, y = fits.copy.copy$lat, colour = as.factor(state)), size = 0.8) + facet_grid(. ~ year) +
  scale_colour_manual(values = c("#009E73", "#000000", "#616161"), name = "Behavioural state",
                      guide = guide_legend(title.position = "top")) +
  scale_shape_manual(values = c(19, 1, 3), name = "Behavioural state",
                     guide = guide_legend(title.position = "top"))
