...
# Get EEZ
dir <- "C:/Users/Ryan Reisinger/Documents/Academic/PEI_Toppredators/Datavis&Mapping"
...
p2 <- p2 + geom_point(data = fits.copy, aes(x = fits.copy$lon, y = fits.copy$lat, colour = as.factor(state)), size = 0.8) + facet_grid(. ~ year) +
  scale_colour_manual(values = c("#e41a1c", "#377eb8", "#616161"), name = "Behavioural state")
...
ggplot(fits.copy, aes(x = lat) + geom_bar()
